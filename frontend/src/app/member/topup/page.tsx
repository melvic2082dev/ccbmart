...
export default function MemberTopup() {
  ...
  const handleSubmit = async () => {
    if (amount < 10000) { setError('So tien toi thieu 10,000 VND'); return; }
      ...
      <DashboardLayout role="member">
        <Card className="max-w-md mx-auto">
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-xl font-bold mb-2">Yeu cau nap tien da gui!</h3>
            <p className="text-gray-500 mb-4">So tien: {formatVND(result.amount)}</p>
            {result.bankAccount && (
              <div className="bg-blue-50 p-4 rounded-lg text-left text-sm mb-4">
                <p className="font-semibold mb-1">Thong tin chuyen khoan:</p>
                ...
                <p>Noi dung: <strong>{result.bankAccount.transferContent}</strong></p>
              </div>
            )}
            <p className="text-xs text-gray-400">Admin se xac nhan khi nhan duoc tien.</p>
            <Button onClick={() => { setResult(null); setAmount(0); }} variant="outline" className="mt-4">Nap them</Button>
          </CardContent>
        </Card>
      </DashboardLayout>
    );
  ...
  return (
    <DashboardLayout role="member">
      <h2 className="text-2xl font-bold mb-6 flex items-center gap-2"><Banknote size={24} /> Nap tien vao vi</h2>
      ...
      <Card className="max-w-md mx-auto">
        <CardContent className="pt-6 space-y-4">
          ...
          <Button onClick={handleSubmit} disabled={loading || amount < 10000} className="w-full">
            {loading ? 'Dang xu ly...' : `Nap ${formatVND(amount)}`}
          </Button>
        </CardContent>
      </Card>
    </DashboardLayout>
